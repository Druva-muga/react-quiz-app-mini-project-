...
const Quiz = () => {
    const { showQuiz, question, quizs, checkAnswer, correctAnswer,
            selectedAnswer,questionIndex, nextQuestion, showTheResult,showAnswerTimer,setShowAnswerTimer,seconds }  = useContext(DataContext);
    ...
    return (
        <section id="quiz" className="text-black" style={{ display: `${showQuiz ? 'block' : 'none'}` }}>
            
            <div className="container">
                <div className="row vh-100 align-items-center justify-content-center">
                    <div className="col-lg-8">
                        <div className="card p-4" style={{ background: '#fff', borderColor: '#646464' }}>
                          {showQuiz && showAnswerTimer && <AnswerTimer duration={6} onTimeUp={handleTimeUp}/>}
                            ...
                            <div>
                                {
                                    question?.options?.map((item, index) => <button
                                        key={index}
                                        className={`option w-100 text-start btn py-2 px-3 mt-3 rounded ${correctAnswer === item && 'bg-success'}`}
